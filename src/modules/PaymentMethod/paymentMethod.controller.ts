...
import { createPaymentMethodIntoDB, deletePaymentMethodFromDB, getAllPaymentMethodsFromDB } from "./paymentMethod.service";
...
export const deletePaymentMethod = catchAsync(async (req: CustomRequest, res: Response) => {
    const { id: userId } = req.user;
    const { paymentMethodId } = req.params;
    const result = await deletePaymentMethodFromDB(userId, paymentMethodId);
    sendResponse(res, {
        statusCode: httpStatus.CREATED,
        success: true,
